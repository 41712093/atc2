...
class AtendimentoService extends MetaModelService
{
	...
	public function chamar_pausado_service(Atendimento $atendimento, Usuario $usuario, $local)
    {
        AppConfig::getInstance()->hook('attending.pre-call', array($atendimento, $usuario, $local));
        ...
        try {
            ...
            $atendimento->setUsuario($usuario);
            $atendimento->setLocal($local);
            $atendimento->setStatus(self::ATENDIMENTO_ENCERRADO);
            $atendimento->setDataChamada(new DateTime());
			$atendimento->setEstadopausado('0');
    ...
    public function atendimentoAndamento($usuario)
    {
        ...
        try {
            return $this->em
                ->createQuery("SELECT e FROM Novosga\Model\Atendimento e WHERE e.usuario = :usuario AND e.status IN (:status) AND e.estadoPausado <> :estadoPausado")
    ...
    public function redirecionar(Atendimento $atendimento, Usuario $usuario, $unidade, $servico)
    {
		...
		$novo->setEstadopausado($atendimento->getEstadopausado());
