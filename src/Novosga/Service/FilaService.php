...
class FilaService extends ModelService
{
    ...
    public function atendimentos(UsuarioSessao $usuario, $maxResults = 0)
    {
	    ...
	    public function atendimentos_pausados(UsuarioSessao $usuario, $maxResults = 0)
    {
        $ids = array(0);
        $servicos = $usuario->getServicos();
        foreach ($servicos as $s) {
        ...
        $cond = '';
        // se nao atende todos, filtra pelo tipo de atendimento
        if (1 != UsuarioSessao::ATEND_TODOS) {
            $s = ($usuario->getTipoAtendimento() == UsuarioSessao::ATEND_CONVENCIONAL) ? '=' : '>';
            $cond = "p.peso $s 0";
            $rs = $this->atendimentosUsuario_pausados($usuario, $ids, $maxResults, $cond);
        } else {
            // se atende todos mas tem limite para sequencia de tipo de atendimento
            $maxPrioridade = (int) AppConfig::getInstance()->get('queue.limits.priority');
            if ($maxPrioridade > 0 && $usuario->getSequenciaPrioridade() > 0 && $usuario->getSequenciaPrioridade() % $maxPrioridade === 0) {
            ...
            // se a lista veio vazia, tenta pegar qualquer um
            if (sizeof($rs) === 0) {
                $rs = $this->atendimentosUsuario_pausados($usuario, $ids, $maxResults);
            }
        }
    ...
    private function atendimentosUsuario(UsuarioSessao $usuario, $servicos, $maxResults = 0, $where = '')
    {
        $builder = $this->builder()
                ->where('e.status = :status AND su.unidade = :unidade AND s.id IN (:servicos) AND e.estadoPausado <> :estadoPausado')
        ;
        if (!empty($where)) {
	...
	private function atendimentosUsuario_pausados(UsuarioSessao $usuario, $servicos, $maxResults = 0, $where = '')
    {
        $builder = $this->builder()
                ->where('e.status = :status AND su.unidade = :unidade AND s.id IN (:servicos) AND e.estadoPausado = :estadoPausado AND e.usuario = :usuario'  )
        ;
        if (!empty($where)) {
            $builder->andWhere($where);
        }
