...
class AtendimentoController extends ModuleController
{
	...
	/*NUEVOS*/
	    public function ajax_update_pausados(Context $context)
    {
        $response = new JsonResponse();
        $unidade = $context->getUnidade();
        $usuarioSessao = $context->getUser();
        if ($unidade && $usuarioSessao) {
            ...
            // fila de atendimento do atendente atual
            $response->data = array(
                'atendimentos' => $this->filaService->atendimentos_pausados($usuarioSessao),
                'usuario' => array(
                    'numeroLocal' => $usuarioSessao->getLocal(),
                    'tipoAtendimento' => $usuarioSessao->getTipoAtendimento(),
                ),
	...
	public function chamar_2(Context $context)
    {
        $response = new JsonResponse();
        try {
            if (!$context->request()->isPost()) {
            ...
            if ($atual) {
                ...
                do {
                    $atendimentos = $this->filaService->atendimentos_pausados($usuario, 1);
                    if (sizeof($atendimentos)) {
                        $proximo = $atendimentos[0];
						
                        $success = $this->atendimentoService->chamar_pausado_service($proximo, $usuario->getWrapped(), $usuario->getLocal());
                        if ($success) {
                            // incrementando contadores
                            if ($proximo->getPrioridade()->getPeso() > 0) {
                                $usuario->setSequenciaPrioridade($usuario->getSequenciaPrioridade() + 1);
	    ...
	    public function cambiaServicio2(Context $context)
    {
		$response = new JsonResponse();
        $unidade = $context->getUnidade();
        //$usuarioSessao = $context->getUser();
	...
	/**
	Codigo para pausar la atencion	
	
	*/
	public function pausar_atencion(Context $context)
    {
        ...
        try {
            ...
            $response->success = $this->mudaStatusUpdatePausados($atual, AtendimentoService::ATENDIMENTO_ENCERRADO, AtendimentoService::ATENDIMENTO_ENCERRADO_CODIFICADO, 'dataFim');
    ...
    private function mudaStatusUpdatePausados(Atendimento $atendimento, $statusAtual, $novoStatus, $campoData)
    {
        $cond = '';
        if ($campoData !== null) {
            $cond = ", e.$campoData = :data";
        ...
        $query->setParameter('novoStatus', '4');
        $query->setParameter('id', $atendimento->getId());
        $query->setParameter('statusAtual', $statusAtual);
		$query->setParameter('estadoPausado', '5');
