...
SGA.Atendimento = {
    ...
    init: function(status) {
		...
		SGA.Atendimento.ajaxUpdate_pausados();
    ...
    ajaxUpdate: function() {
        ...
        if (!SGA.paused) {
            SGA.ajax({
                ...
                complete: function() {
                    SGA.Atendimento.timeoutId = setTimeout(SGA.Atendimento.ajaxUpdate_pausados, SGA.updateInterval);
                }
            });
    ...
    chamar: function(btn) {
        ...
        SGA.Atendimento.control({
            ...
            success: function(response) {
                ...
                if (response.data.senha == proximo.text()) {
                    ...
                    if ($("#fila ul li").length == 0) {
                        // fila vazia
                        $("#fila ul").append('<li class="empty_pausados">' + SGA.Atendimento.filaVazia + '</li>')
                    } else {
                        // novo proximo
                        $("#fila ul li:first a").addClass('proximo'); 
                    }
	 ...
	 cambiaServicio: function(btn) {
		    SGA.Atendimento.control({
            button: btn,
            enableDelay: 5000,
            action: 'cambiaServicio2', 
            success: function(response) {
				var hola = response.data.usuario.tipoAtendimento;
              alert (hola);
            }
        });
	...
	pausar_atencion: function(btn) {
		...
		if (window.confirm('Realmente desea marcar pausar la atención?')) {
            SGA.Atendimento.control({
            button: btn,
            action: 'pausar_atencion',
			data: data,			
            success: function() {
                //SGA.Atendimento.updateControls(4, response.data)
	...
	/*NUEVOS*/
    ajaxUpdate_pausados: function() {
        clearTimeout(SGA.Atendimento.timeoutId);
        if (!SGA.paused) {
            SGA.ajax({
                url: SGA.url('ajax_update_pausados'),
                success: function(response) {
                    response.data = response.data || {};
                    var atendimentos = response.data.atendimentos || [],
                            usuario = response.data.usuario || {};
                    var list = $("#fila_pausados ul");
                    // habilitando botao chamar
                    if (atendimentos.length > 0) {
                        $('#chamar_2 .chamar_2').prop('disabled', false);
                        // se a fila estava vazia e chegou um novo atendimento, entao toca o som
                        if (list.find('li.empty').length > 0) {
                            document.getElementById("alert").play();
                            SGA.Notification.show('Atendimento', 'Novo atendimento na fila');
                    ...
                    if (atendimentos.length > 0) {
                        ...
                        $('#chamar_2 .chamar_2').prop('disabled', true);
                        list.append('<li class="empty">' + SGA.Atendimento.filaVazia + '</li>');
                        document.title = SGA.Atendimento.defaultTitle;
                    }
                    if (usuario.numeroLocal) {
